...
//		hb.open();
	...
	/**
	 * (Jhovanny) - Consulta por id, devuelve el departamento
	 * @param id
	 * @return
	 */
	public Proyecto findById2(Integer id) {
        logger.info("findById()");
	 ...
	 * @return
	 */
	public boolean update(Proyecto p, Integer id) {
		logger.info("update()");
		hb.getTransaction().begin();
		try {
		p = hb.getManager().find(Proyecto.class, id);
